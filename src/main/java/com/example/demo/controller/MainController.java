...
public class MainController {
    ...
    public List<TestDto> insertData() {
    ...
    public String getData1(@RequestParam("areaCd") String areaCd, @RequestParam("sigunguCd") String sigunguCd,
            @RequestParam("baseYm") String baseYm) throws Exception {
    ...
    public String getData2(@RequestParam("areaCd") String areaCd, @RequestParam("sigunguCd") String sigunguCd,
            @RequestParam("baseYm") String tAtsNm) throws Exception {
        ...
        System.out.println(params);
        // return
        // getData("TatsCnctrRateService/tatsCnctrRatedList","&areaCd=51&signguCd=51130&tAtsNm=",1,10);
    ...
    public String getData3() throws Exception {
        ...
        // return
        // getData("Durunubi/courseList","&routeIdx=T_ROUTE_MNG0000000006&crsKorNm=%EB%B0%80%EC%96%91%EA%B0%95&routeIdx=%20&crsLevel=2&brdDiv=DNBW",1,10);
    }

    // 투어 빅데이터인데 두개로 나누어짐. 광역 지자체 지역 방문자수 / 기초 지자체 지역방문자수
    @GetMapping("/getTourBigData")
    public String getData4(@RequestParam("startYmd") String startYmd, @RequestParam("endYmd") String endYmd)
            throws Exception {
        // 지역 지자체 지역방문자수 집계 데이터 정보 조회
        // startYmd=20210513&endYmd=20210513
        // return
        // getData("DataLabService/locgoRegnVisitrDDList","&startYmd=20210513&endYmd=20210513",1,100);
        // 광역 지자체 지역방문자수 집계 데이터 정보 조회
        String params = "startYmd=" + startYmd + "&endYmd=" + endYmd;
        System.out.println(params);
        // return
        // getData("DataLabService/metcoRegnVisitrDDList","&startYmd=20210513&endYmd=20210513",1,10);
    ...
    public String getData5() throws Exception {
        ...
        // 위치기반 목록 조회
        // return
        // getData("GoCamping/locationBasedList","mapX=128.6142847&mapY=36.0345423&radius=2000",1,100);
        // 키워드 검색 목록 조회
        // return
        // getData("GoCamping/searchList","keyword=%EC%95%BC%EC%98%81%EC%9E%A5",1,100);
        // 요청변수
    ...
    public String getData6(@RequestParam("baseYm") String baseYm, @RequestParam("areaCd") String areaCd,
            @RequestParam("sigunguCd") String sigunguCd, @RequestParam("keyword") String keyword) throws Exception {
        // 지역기반 관광지별 연관 과노강지 정보 목록 조회
        // return
        // getData("/TarRlteTarService/areaBasedList","&baseYm=202408&areaCd=11&signguCd=11530",1,100);
        String params = "baseYm=" + baseYm + "&areaCd=" + areaCd + "&sigunguCd=" + sigunguCd + "&keyword" + keyword;
        System.out.println(params);
        // 키워드 검색 관광지별 연관 관광지 정보 목록 조회
        return getData("TarRlteTarService/searchKeyword",
                "baseYm=202408&areaCd=51&signguCd=51130&keyword=%EB%AE%A4%EC%A7%80%EC%97%84%EC%82%B0", 1, 10);
    }
    ...
    public String getData(String link, String params, int pageNo, int numOfRows) throws Exception {
        ...
        String apiUrl = "https://apis.data.go.kr/B551011/" + link + "?serviceKey=" + servicekey + "&pageNo=" + pageNo
                + "&numOfRows=" + numOfRows + "&MobileOS=ETC&MobileApp=AppTest&" + params + "&_type=json";
        // HTTP 요청 생성
